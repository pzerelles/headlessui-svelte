...
export async function click(element: Document | Element | Window | Node | null, button = MouseButton.Left) {
  return act(() => rawClick(element, button))
}

export async function rawClick(element: Document | Element | Window | Node | null, button = MouseButton.Left) {
  try {
    if (element === null) return expect(element).not.toBe(null)
